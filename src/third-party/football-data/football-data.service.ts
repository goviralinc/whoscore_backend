...
export class FootballDataService {
    ...
    async getCompetitions() {
        const competionsUrl = `${this.baseUrl}/competitions/`;
        const response = await this.sendRequest(competionsUrl);
        const competitions = this.competitionMapper.mapToDtoArray(response.competitions);
        
        return competitions
    }

    async getLiveScores(competition: string) {
        let matchesUrl = `${this.baseUrl}/matches?status=IN_PLAY`;

        // If competition is provided, add it to the URL
        if (competition) {
            matchesUrl = `${this.baseUrl}/competitions/${competition}/matches?status=IN_PLAY`;
        }
    ...
    async getFixtures(competition: string, date: string) {
        let fixturesUrl = `${this.baseUrl}/competitions/${competition}/matches?`;
